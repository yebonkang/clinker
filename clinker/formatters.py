...
def get_link_values(link, decimals=4, show_mismatches=False):
    values = [
        link.query.label,
        link.target.label,
    ...
    if show_mismatches:
        mismatches = "" if link.mismatches is None else str(int(link.mismatches))
        values.append(mismatches)
    return values
...
def format_links(
    ...
    """
    rows = [
        get_link_values(link, decimals=decimals, show_mismatches=show_mismatches)
        for link in links
    ]
    if headers:
        hdrs = ["Query", "Target", "Identity", "Similarity"]
        if show_mismatches:
            hdrs.append("Mismatches")
        rows.insert(0, hdrs)
    if not delimiter:
        delimiter = "  "
        rows = humanise(rows)
